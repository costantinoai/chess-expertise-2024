...
def get_roi_mapping(csv_path):
    df = pd.read_csv(csv_path)
    ROI_MAPPING = df.groupby('cortex')['region'].agg(list).to_dict()        
    return ROI_MAPPING

# # Mapping of smaller ROIs to bigger ROIs based on the given text
# ROI_MAPPING = {
#     "VC": [
#         "V1", "V2", "V3", "V3A", "V3B", "V3CD", "V4", "V4t", "V6", "V6A", "V7", "V8", 
#         "FST", "IPS1", "FFC", "LO1", "LO2", "LO3", "PH", "PIT", "MT", "MST", "VMV1", 
#         "VMV2", "VMV3", "VVC"
#     ],
#     "TPJ": ["TPOJ1", "TPOJ2", "TPOJ3"],
#     "IPL": ["PFm", "PGi", "PGs"],
#     "PC": ["7m", "v23ab", "d23ab", "31pv", "31pd", "31a", "PCV"],
#     "STS": ["STSda", "STSdp", "STSva", "STSvp"],
#     "TE": ["TE1a", "TE1m", "TE1p", "TE2a"],
#     "MTC": ["EC", "H"],
#     "Insula": ["AAIC", "MI", "PoI1", "PoI2", "FOP2", "FOP3", "Ig", "OP2-3"],
#     "DLPFC": ["9-46d", "46", "a9-46v", "p9-46v"],
#     "DMPFC": ["d32", "9m", "10d"],
#     "VMPFC": ["10r", "10v", "p32", "s32"],
#     "ACC": ["a24", "a24pr", "p24", "a32pr", "p32pr"],
#     "OFC": ["OFC", "pOFC"]
# }
...
def calculate_stats(dfs, chance_levels, multiple_comp_corr='fdr'):
    ...
    for idx in stats_df.index:
        ...
        for roi in stats_df.columns.levels[0]:
            ...
            # Calculate the two-tailed t-test on a single sample
            t_stat, p_value_two_tailed = stats.ttest_1samp(concatenated_df.loc[idx, roi], popmean=0.0, nan_policy='omit')
            
            # Convert the two-tailed p-value to a one-tailed p-value
            # Assuming the alternative hypothesis is that the mean is greater than the population mean
...
csv_path='/home/eik-tb/Desktop/test/HCP-MMP1_UniqueRegionList.csv'
...
for multiple_comparions_corr in [True, False]:
    ...
    expert_averages_df, expert_ci_lower_df, expert_ci_upper_df, expert_p_values_df = (
        calculate_stats(experts_dfs_roigroup, chance_levels=[0.5, 0.05, 0.1], multiple_comp_corr=fdr_string)
    )
    (
    ...
    ) = calculate_stats(nonexpert_dfs_roigroup, chance_levels=[0.5, 0.05, 0.1], multiple_comp_corr=fdr_string)
