...
for subject_id, acts in zip(["exp", "nonexp"], [acts_trained, acts_untrained]):
    for roi, roi_data in acts.items():
        ...
        feats_obs = np.stack(all_activations, axis=1)  # features × observations

        # Now feats_obs is ready for manifold analysis
        # (you can continue here with dimensionality cleaning, etc.)
