...
def process_single_roi(subject_id, roi, roi_data, classes):
    ...
    if len(X_list) == 0:
        logger.warning(f"[{subject_id}] Skipping ROI {roi}: no classes with >= 2 samples.")
        return roi, None

    # Perform manifold analysis
    a, r, d, rho0, K = manifold_analysis_corr(X_list, kappa=0, n_t=200, n_reps=10)
...
for subject_id, acts in zip(["exp", "nonexp"], [acts_trained, acts_untrained]):
    ...
    if USE_PARALLEL:
        ...
        roi_jobs = Parallel(n_jobs=N_JOBS, backend="loky")(
            delayed(process_single_roi)(subject_id, roi, roi_data, classes)
        ...
        for roi, roi_data in acts.items():
            roi, result = process_single_roi(subject_id, roi, roi_data, classes)
            if result is not None:
                roi_results[roi] = result
