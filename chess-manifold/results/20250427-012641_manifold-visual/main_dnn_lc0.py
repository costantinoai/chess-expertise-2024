...
file_path = '/home/eik-tb/OneDrive_andreaivan.costantino@kuleuven.be/GitHub/AlphazeroChess/lc0_analysis/act_lc0_reorg/activations_model-trained_seed-0.pkl'

# Open and load the pickle file
with open(file_path, 'rb') as f:
    acts_trained = pickle.load(f)
...
def process_single_roi(subject_id, roi, roi_data):
    if "value" in roi:
        return roi, None  # Skip non-ROI entries

    logger.info("[%s] Starting manifold analysis (%s ROI)", subject_id, roi)

    all_activations = {}
    for stim_id, stim_info in roi_data.items():
        activation = stim_info['activation']
    ...
    stim_ids = sorted(all_activations.keys())
    feats_obs = np.stack(
        [all_activations[stim_id] for stim_id in stim_ids],
        axis=0
    )
    ...
    # 2. Drop observations (rows) that are all-zero
    nonzero_mask = ~np.all(feats_obs == 0, axis=1)
    feats_clean = feats_obs[nonzero_mask, :]

    if feats_clean.shape[0] == 0 or feats_clean.shape[1] == 0:
        logger.warning("[%s] Skipping ROI %s: empty after cleaning.", subject_id, roi)
        return roi, None

    # Prepare X_list: each sample reshaped properly
    X = [d.reshape(d.shape[0], -1) for d in feats_clean]

    # Random projection if too high dimensional
    N = X[0].shape[0]
    if N > 5000:
        print(f"[{subject_id}] Projecting ROI {roi} to 5000 dimensions")
    ...
    if len(X_list) == 0:
        logger.warning("[%s] Skipping ROI %s: no valid data after grouping.", subject_id, roi)
        return roi, None

    # Interleave first 20 and next 20
    X_list = [
        np.concatenate([X_list[i], X_list[i+20]], axis=1)  for i in range(20)
    ]
    try:
        # Manifold Analysis
        ...
        logger.error("[%s] Manifold analysis failed for ROI %s: %s", subject_id, roi, str(e))
        return roi, None
    ...
    return roi, roi_result

# ===== Main analysis =====
USE_PARALLEL = True
for subject_id, acts in zip(["exp"], [acts_trained]):
    roi_results = {}

    if USE_PARALLEL:
        logger.info("[%s] Processing ROIs in parallel", subject_id)
        roi_jobs = Parallel(n_jobs=N_JOBS, backend="loky")(
            delayed(process_single_roi)(subject_id, roi, roi_data)
            for roi, roi_data in acts.items()
        )
        for roi, result in roi_jobs:
            if result is not None:
                roi_results[roi] = result
    else:
        logger.info("[%s] Processing ROIs serially", subject_id)
        for roi, roi_data in acts.items():
            roi, result = process_single_roi(subject_id, roi, roi_data)
            if result is not None:
                roi_results[roi] = result
