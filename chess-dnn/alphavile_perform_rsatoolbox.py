...
def plot_delta_rsa(
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(layers, ha='right')

    # Labels and title
    title = regressor_mapping.get(model_name, model_name)
    subtitle = " (Checkmate stimuli)" if key == "half" else ""
    ax.set_title(f"Model predictivity  -  {title}{subtitle}", pad=45)
    ax.set_xlabel("Layer")
    ax.set_ylabel("ΔRSA (Trained − Untrained)")
    ...
    fname = f"{model_name.replace('/', '-')}_{key}_rsa_diff.png"
