...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 21 00:14:51 2025
...
class Parcellation:
    ...
    def plot_surface(self, level: int = 0, color_by: Optional[str] = None):
        ...
        # Compose ROI label vector for left hemisphere only (for demo simplicity)
        surf = self.meshes["inflated"]
        n_vertices = surf.parts["left"].coordinates.shape[0]
        label_vec = np.full(n_vertices, -1)
        for parcel in self.get_parcels_by_level(level):
            if parcel.hemisphere == "left":
                label_vec[parcel.vertices] = parcel.label

        surface = SurfaceImage(mesh=surf, data={"left": label_vec})
        fig = plot_surf_roi(
            roi_map=surface,
            bg_map=fsaverage["inflated"],
            hemi="left",
            view="lateral",
            bg_on_data=True,
            engine="plotly",
        ...
        logger.info(f"Opening brain surface plot (level={level})...")
        fig.figure.show()
        return fig
...
class BrainViewer:
    def __init__(self, parcellation, port: int = 8050):
        ...
        self.port = port
        self.sulcal_data = load_fsaverage_data("fsaverage", data_type="sulcal").data.parts
    ...
    def _register_callbacks(self):
        ...
        @self.app.callback(
            ...
            Input("mesh-type-dropdown", "value"),  # <-- NEW
            State("brain-3d", "relayoutData"),  # <- correctly captured
        )
        def update_surface(level, hemis, rsa_toggle, rsa_regressor, mesh_type, relayoutData):
            coords_all, faces_all, hovertext_all, customdata_all, vertex_rgb_all = [], [], [], [], []
            offset = 100.0
            total_offset = 0
            rsa_mode = "rsa" in rsa_toggle and rsa_regressor is not None
            cmap = colormaps["seismic"]

            # --- Preserve camera view if available ---
            camera = None
            ...
            for hemi in ["left", "right"]:
                ...
                coords, faces = self.parcellation.meshes[mesh_type][hemi]
                coords = coords.copy()
                if hemi == "right":
                    coords[:, 0] += offset
                ...
                vertex_rgb = np.full((n_verts, 3), np.nan)
                ...
                for p in self.parcellation.get_parcels_by_level(level):
                    ...
                    if rsa_mode:
                        rsa = p.extra.get("rsa", {}).get(rsa_regressor, {})
                        # if rsa:
                        if rsa and rsa.get("fdr_reject", False):
                        # if rsa and rsa.get("p_uncorrected") < .05:
                            val = rsa.get("mean_diff", 0.0)
                            # Normalize to [0,1] using symmetric vmin/vmax
                            norm_val = (val - self.vmin) / (self.vmax - self.vmin) if self.vmax > self.vmin else 0.5
                            norm_val = np.clip(norm_val, 0.0, 1.0)
                            rgba = cmap(norm_val)
                            vertex_rgb[p.vertices] = rgba[:3]
                    else:
                        if p.color:
                            vertex_rgb[p.vertices] = p.color[:3]

                nan_mask = np.isnan(vertex_rgb).any(axis=1)
                vertex_rgb[nan_mask] = sulc_rgb[nan_mask]

                coords_all.append(coords)
                faces_all.append(faces + total_offset)
                hovertext_all.append(hovertext)
                customdata_all.append(customdata)
                vertex_rgb_all.append(vertex_rgb)
                total_offset += n_verts

            coords = np.vstack(coords_all)
            faces = np.vstack(faces_all)
            hovertext = np.concatenate(hovertext_all)
            customdata = np.concatenate(customdata_all)
            vertex_rgb = np.vstack(vertex_rgb_all)
            ...
            fig.update_layout(
                scene=dict(
                    ...
                    camera=camera or {},  # preserve camera if available
                ),
...
meta_csv = "/home/eik-tb/OneDrive_andreaivan.costantino@kuleuven.be/GitHub/chess-expertise-2024/chess-rois/data/HCP-MMP1_UniqueRegionList.csv"

# Base parcellation
base_parcellation = GlasserParcellation()
base_parcellation.load_from_glasser_files(annot_paths, lut_paths, meta_csv)

# --- Load Trained RSA results ---
trained_pkl = "/data/projects/chess/data/BIDS/derivatives/mvpa-dnn/20250421-010758_glasser_cortex_bilateral/rsa_corr_trained/20250421-013617_group/ttest_group_results.pkl"
with open(trained_pkl, "rb") as f:
    trained_results = pickle.load(f)

# --- Load Untrained RSA results ---
untrained_pkl = "/data/projects/chess/data/BIDS/derivatives/mvpa-dnn/20250421-010758_glasser_cortex_bilateral/rsa_corr_untrained/20250421-013623_group/ttest_group_results.pkl"
with open(untrained_pkl, "rb") as f:
    untrained_results = pickle.load(f)

# --- Make separate parcellation copies ---
trained_parcellation = deepcopy(base_parcellation)
untrained_parcellation = deepcopy(base_parcellation)

# --- Assign RSA results from "experts_vs_nonexperts" group key ---
trained_parcellation.assign_rsa_stats(trained_results["experts_vs_nonexperts"])
untrained_parcellation.assign_rsa_stats(untrained_results["experts_vs_nonexperts"])
# untrained_parcellation.assign_rsa_stats(untrained_results["experts_vs_nonexperts"])

# --- View both interactively (separate browser tabs) ---
trained_viewer = BrainViewer(trained_parcellation, port=8050)
untrained_viewer = BrainViewer(untrained_parcellation, port=8051)

trained_viewer.run()
untrained_viewer.run()
