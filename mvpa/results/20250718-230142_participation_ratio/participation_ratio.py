...
#         18: "Posterior Cing", 19: "Anterior Cing and Medial Prefrontal", 20: "Orbital and Polar Frontal",
#         21: "Inferior Frontal", 22: "Dorsolateral Prefrontal"
#     }

#     # Merge and map ROI names
...
#         }
#     else:
#         palette_dict = sns.color_palette("husl", len(roi_names))
...
#             fmt='none', ecolor='black', elinewidth=1.5, capsize=2, zorder=2
#         )
...
#     ax.set_ylabel("Participation Ratio Δ (Experts - Non-Experts)")
#     ax.set_xlabel("")
#     ax.set_title(f"Participation Ratio Differences ({'FDR' if use_fdr else 'Raw'} p < 0.05)", pad=20)
...
#     # Tick color by significance
#     bar_colors = {roi: patch.get_facecolor() for roi, patch in zip(roi_names, ax.patches)}
#     for label in ax.get_xticklabels():
#         roi = label.get_text()
#         is_sig = merged.loc[merged["ROI_Name"] == roi, "significant_fdr"].values[0]
#         label.set_color("lightgrey" if not is_sig else bar_colors.get(roi, "black"))
...
def plot_grouped_roi_bars_with_dots(
    ...
    for roi_label in roi_labels:
        ...
        # Means
        expert_means.append(np.mean(expert_data))
        nonexpert_means.append(np.mean(nonexpert_data))

        # One-sample t-test to get 95% CI of the mean
        expert_ttest = ttest_1samp(expert_data, popmean=0)
        nonexpert_ttest = ttest_1samp(nonexpert_data, popmean=0)

        expert_cis.append(expert_ttest.confidence_interval(confidence_level=0.95))
        nonexpert_cis.append(nonexpert_ttest.confidence_interval(confidence_level=0.95))
    ...
    for i, roi in enumerate(roi_names):
        ...
        if is_sig[i]:
            y_top = max(expert_means[i] + expert_cis[i], nonexpert_means[i] + nonexpert_cis[i])
    ...
    # Assemble new DataFrame for plotting
    diff_data = []
    for i, roi in enumerate(roi_names):
        ci_low = expert_cis[i].low - nonexpert_cis[i].high
        ci_high = expert_cis[i].high - nonexpert_cis[i].low
        diff = expert_means[i] - nonexpert_means[i]
        diff_data.append({
            "ROI_Label": roi_labels[i],
            "ROI_Name": roi,
            "mean_diff": diff,
            "ci95_low": diff - (diff - ci_low),
            "ci95_high": diff + (ci_high - diff),
