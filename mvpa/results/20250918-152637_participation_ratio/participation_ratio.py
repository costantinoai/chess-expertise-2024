...
def plot_pr_combined_panel_from_results(
    ...
    """
    Make a single, manuscript-ready 2-panel figure:
      Top:   Expert vs Novice bars with 95% CIs and significance markers
      Bottom: Δ = Expert - Novice bars with 95% CIΔ and significance markers
    ...
    results_df : pd.DataFrame
        Output of build_pr_results_df(...): must contain
        ['ROI_Label','ROI_Name',
         'expert_mean','expert_ci_low','expert_ci_high',
         'novice_mean','novice_ci_low','novice_ci_high',
         'delta_mean','delta_ci_low','delta_ci_high',
         'p_raw','p_fdr'].
    measure_name : str
        Used in axis labels / filename (e.g., "PR").
    output_dir : str
        Where to save the PNG.
    use_fdr : bool
        If True, stars use p_fdr; otherwise p_raw.
    sort_by : {"roi","diff"}
        "roi" → sort by ROI_Label; "diff" → sort by delta_mean desc.
    custom_colors : list or None
        If provided, one color per ROI (re-used across panels).
    fig_title : str or None
        Figure-level title (above both panels). If None, auto-generates.
    """
    ...
    # ----------- Figure layout -----------
    fig = plt.figure(constrained_layout=True, figsize=(14, 10))  # narrower, balanced height
    ...
    for i, name in enumerate(roi_names):
        ...
        ax_top.bar(
            ...
            label="Experts" if i == 0 else None
        )
        ...
        ax_top.bar(
            ...
            label="Novices" if i == 0 else None
        )
    ...
    ax_bot.set_ylabel(f"Δ{measure_name} = Experts − Novices (±95% CI)")
    ...
    ax_bot.set_ylim(y_min - 0.1, y_max + 0.1)
    ...
    if fig_title is None:
        fig_title = f"{measure_name}: Group means and group differences across ROIs"
    fig.suptitle(fig_title, fontsize=plt.rcParams["axes.titlesize"] * 1.1, y=1.02)
...
if __name__ == "__main__":
    ...
    def plot_pr_bars_compact_combo(
        results_df,
        measure_name,
        output_dir,
        use_fdr=True,
        ...
        """
        One figure with two aligned bar panels:
          Top   : Expert vs Novice means with 95% CIs
          Bottom: Δ = Expert − Novice with 95% CIΔ (+ stars)
        Reads ONLY from `results_df`.
        """
        import numpy as np
        ...
        # pick p-values to use for stars
        pvals = df["p_fdr"].values if use_fdr else df["p_raw"].values
        is_sig = np.array([False if np.isnan(p) else (p < ALPHA_FDR) for p in pvals])

        # palette per ROI
        if custom_colors is not None:
            palette = {name: custom_colors[i % len(custom_colors)] for i, name in enumerate(roi_names)}
        else:
            pal = sns.color_palette("husl", len(roi_names))
            palette = {name: pal[i] for i, name in enumerate(roi_names)}

        # --- figure & axes ---
        fig, (ax_top, ax_bot) = plt.subplots(
            2, 1, figsize=(21, 12), sharex=True,
        ...
        for i, name in enumerate(roi_names):
            ...
            e_mean, e_lo, e_hi = row["expert_mean"], row["expert_ci_low"], row["expert_ci_high"]
            e_err = ci_to_errbar(e_mean, e_lo, e_hi)
            ax_top.bar(x[i] - bar_width/2, e_mean, bar_width,
                       yerr=e_err, capsize=cap, color=c, edgecolor="black",
                       label="Expert" if i == 0 else "", zorder=2)

            # Novice
            n_mean, n_lo, n_hi = row["novice_mean"], row["novice_ci_low"], row["novice_ci_high"]
            n_err = ci_to_errbar(n_mean, n_lo, n_hi)
            ax_top.bar(x[i] + bar_width/2, n_mean, bar_width,
                       yerr=n_err, capsize=cap, color=c, edgecolor="black", hatch="//",
                       label="Novice" if i == 0 else "", zorder=2)

        ax_top.set_ylabel(measure_name)
        ax_top.set_title(f"{measure_name} by ROI (bars = mean; whiskers = 95% CI)")
        ax_top.axhline(0, color="black", linestyle="--", linewidth=1)
        ax_top.legend(loc="upper right", ncol=2, frameon=False)

        # y-lims from CIs with a small buffer
        y_min = np.nanmin(np.r_[df["expert_ci_low"].values, df["novice_ci_low"].values])
        y_max = np.nanmax(np.r_[df["expert_ci_high"].values, df["novice_ci_high"].values])
        ax_top.set_ylim(y_min - 0.1, y_max + abs(y_max)*0.1)
        ...
        sns.barplot(
            ...
            palette=[palette[name] for name in roi_names], ax=ax_bot
        )

        # add CI whiskers and stars
        for i, row in df.iterrows():
            d_mean, d_lo, d_hi = row["delta_mean"], row["delta_ci_low"], row["delta_ci_high"]
            ax_bot.errorbar(
                i, d_mean,
                yerr=[[d_mean - d_lo], [d_hi - d_mean]],
                fmt="none", ecolor="black", elinewidth=1.5, capsize=cap, zorder=2
            )
            if is_sig[i]:
                y_pos, va = (d_hi + sig_offset, "bottom") if d_mean >= 0 else (d_lo - sig_offset, "top")
                ax_bot.text(i, y_pos, "*", ha="center", va=va,
                            fontsize=plt.rcParams["font.size"] * 1.2, zorder=5)

        ax_bot.axhline(0, color="black", linestyle="--", linewidth=1)
        ax_bot.set_ylabel(f"Δ{measure_name} (Expert − Novice)")
        ax_bot.set_title(f"Group differences ({'FDR' if use_fdr else 'raw'} p < {ALPHA_FDR})")

        # x-axis labels & tint by significance (Δ)
        ax_bot.set_xticks(x)
        ax_bot.set_xticklabels(roi_names, rotation=30, ha="right")
        for tick, name, sig in zip(ax_bot.get_xticklabels(), roi_names, is_sig):
            tick.set_color(palette[name] if sig else "lightgrey")

        # y-lims from Δ CI
        dy_min = np.nanmin(df["delta_ci_low"].values)
        dy_max = np.nanmax(df["delta_ci_high"].values)
        ax_bot.set_ylim(dy_min - 0.1, dy_max + 0.1)
        ...
        plt.tight_layout()
        out = os.path.join(output_dir, f"roi_{measure_name}_bars_compact_combo.png")
