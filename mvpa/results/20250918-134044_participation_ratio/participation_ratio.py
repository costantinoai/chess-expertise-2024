...
def plot_pr_combined_panel_from_results(
    ...
    Make a single, manuscript-ready 2-panel figure:
      (A) Expert vs Novice bars with 95% CIs and significance markers
      (B) Δ = Expert - Novice bars with 95% CIΔ and significance markers
    ...
    panel_labels : bool
        If True, prepend "(A)" and "(B)" to panel titles.
    """
    ...
    # ------------- Figure + layout -------------
    # Taller top panel for means; shorter bottom for deltas
    fig = plt.figure(constrained_layout=True, figsize=plt.rcParams["figure.figsize"])
    gs = fig.add_gridspec(nrows=2, ncols=1, height_ratios=[3.0, 2.0])
    ...
    for i, name in enumerate(roi_names):
        ...
        # Star for significance (based on chosen p)
    ...
    # Axis, style
    ax_top.set_ylabel(f"{measure_name} (group means)")
    title_top = "Group means ± 95% CI"
    if panel_labels:
        title_top = f"(A) {title_top}"
    ax_top.set_title(title_top)
    ax_top.axhline(0, color="black", linestyle="--", linewidth=1)

    # Tint tick labels by significance (top uses same x, but we’ll draw bottom labels only)
    for i, name in enumerate(roi_names):
        # We’ll color bottom ticks; hide top x ticklabels for clarity
        pass
    ax_top.tick_params(axis="x", labelbottom=False)

    # Y-lims from CIs with buffer
    ...
    ax_top.legend(loc="upper right", bbox_to_anchor=(1, 1.15), ncol=2, frameon=False)
    ...
    # Axis, style
    ax_bot.set_ylabel(f"Δ{measure_name} (Exp − Nov)")
    title_bot = f"Differences ± 95% CI ({'FDR' if use_fdr else 'raw'} p < {ALPHA_FDR})"
    if panel_labels:
        title_bot = f"(B) {title_bot}"
    ax_bot.set_title(title_bot, pad=12)
    ax_bot.axhline(0, color="black", linestyle="--", linewidth=1)

    # Bottom x labels (one place only)
    ax_bot.set_xticks(x)
...
def plot_grouped_roi_bars_with_dots_from_results(
    ...
    """
    Plot group bars with CIs and Δ bars with CIΔ using precomputed `results_df`.
    No statistical computations happen here.
    """
    # Sort by ROI index or by effect size
    if sort_by == "roi":
        df = results_df.sort_values("ROI_Label", key=natsorted).copy()
    elif sort_by == "diff":
        df = results_df.sort_values("delta_mean", ascending=False).copy()
    ...
    for i, name in enumerate(roi_names):
        ...
        # star if significant (based on chosen p)
        if is_sig[i]:
            y_top = max(e_hi, n_hi)
            y_line = y_top + sig_offset * 1.2
            y_ast  = y_line + sig_offset * 0.05
            ax.plot([x_coords[i] - bar_width/2, x_coords[i] + bar_width/2], [y_line, y_line],
                    color="black", linewidth=1.2)
    ...
    plt.tight_layout()
    out1 = os.path.join(output_dir, f"roi_bars_{measure_name}_grouped_with_dots.png")
    ...
    # --- Figure 2: Δ bars + CIΔ ---
    fig, ax = plt.subplots()
    sns.barplot(x="ROI_Name", y="delta_mean", data=df,
                palette=[palette_dict[name] for name in roi_names], ax=ax)

    # add CI whiskers
    for i, row in df.iterrows():
        d_mean, d_lo, d_hi = row["delta_mean"], row["delta_ci_low"], row["delta_ci_high"]
        ax.errorbar(x=i, y=d_mean,
                    yerr=[[d_mean - d_lo], [d_hi - d_mean]],
    ...
    for i, row in df.iterrows():
        if is_sig[i]:
            y_pos, va = (row["delta_ci_high"] + sig_offset, "bottom") if row["delta_mean"] >= 0 \
                        else (row["delta_ci_low"] - sig_offset, "top")
            ax.text(i, y_pos, "*", ha="center", va=va, fontsize=asterisk_fontsize, zorder=5)
    ...
    plt.tight_layout()
    out2 = os.path.join(output_dir, f"roi_diff_{measure_name}_bar_ci.png")
