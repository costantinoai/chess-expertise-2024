...
def plot_feature_importance_from_pca(clf, roi_names, top_n=None):
    ...
    ax.set_title("Top 10 ROI Contributions to Group Classification", pad=60)
    ...
    ax.tick_params(axis='y', colors='lightgray', length=0)  # remove tick marks

    from matplotlib.font_manager import FontProperties
    # Use a font known to support Unicode arrows (like DejaVu Sans)
    arrow_font = FontProperties(family="DejaVu Sans", size=18)

    # Arrow text using a separate font
    ax.text(
        0.5, 1.05,
        "← Lower PR shifts toward Expert  |  Lower PR shifts toward Novice →",
        ha="center",
        va="center",
...
def plot_pr_and_pca_heatmaps(
    ...
    fig, axes = plt.subplots(
        nrows=2,
        ncols=1,
        figsize=(len(roi_names) * 0.4, len(subject_ids) * 0.3 + n_components * 0.6),
        sharex=True,
    ...
    axes[1].set_xticklabels(roi_names, rotation=45, ha="right", color="lightgray")
