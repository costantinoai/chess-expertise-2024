...
def plot_pr_combined_panel_from_results(
    ...
    family_handles = [
        plt.Rectangle((0, 0), 0.5, 1, color=c, edgecolor="black")
        for c in roi_family_colors
    ]

    # bottom legend
    fig.legend(
        handles=family_handles, labels=roi_family_labels,
        loc="lower center", bbox_to_anchor=(0.5, -0.05),
        frameon=True, ncol=7, prop={'size': 20}
    )

    # Make a bit of room for the legend at the bottom
    plt.subplots_adjust(top=0.85, bottom=0.12, hspace=2)
