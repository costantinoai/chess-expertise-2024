...
def color_significant_parcels_on_flat(
    freesurfer_path,
    lh_annotation,
    rh_annotation,
    averages_df,
    ...
    for index, row in averages_df.iterrows():
        ...
        for roi, p_value in zip(averages_df.columns, p_values_list):
            ...
            value = row[roi]
            hemi, label_index = reverse_roi_name_to_annotation_label(
                roi, lh_names, rh_names
            )

            if hemi == "lh":
                vertices_lh = np.where(lh_labels == label_index)[0]
                overlay_data_lh[vertices_lh] = (
                    value if p_value < significance_level else 0.0
                )
            elif hemi == "rh":
                vertices_rh = np.where(rh_labels == label_index)[0]
                overlay_data_rh[vertices_rh] = (
                    value if p_value < significance_level else 0.0
                )
    ...
    fig, axs = plt.subplots(
        nrows=3,
        ncols=len(contrasts) * 2,
        figsize=(4 * len(contrasts) * 2, 12),
        facecolor="black",
    ...
    for index, contrast in enumerate(contrasts):
        for hemi_index, hemi in enumerate(["lh", "rh"]):
            for view_index, view in enumerate(["lateral", "medial", "ventral"]):
                ...
                overlay_data_hemi = (
                    overlay_data[index][0] if hemi == "lh" else overlay_data[index][1]
                )
...
for multiple_comparions_corr in [True, False]:
    ...
    expert_averages_df, expert_ci_lower_df, expert_ci_upper_df, expert_p_values_df = (
        calculate_stats(
            expert_dfs, chance_levels=chance_levels, multiple_comp_corr=fdr_string
        )
    )
    (
        ...
        nonexpert_dfs, chance_levels=chance_levels, multiple_comp_corr=fdr_string
    )
...
fig_width = (
    2 * img_width / 100
)  # 2 images per row, width in inches (100 px/inch is a guess, adjust as needed)
...
fig.savefig(
    os.path.join(root_path, "single_rois", "brain_images_grid.png"),
    dpi=300,
    bbox_inches="tight",
    pad_inches=0,
