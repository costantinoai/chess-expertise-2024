...
from modules.config          import LEVELS_MAPS, COL_POS, COL_NEG, TITLE_FONT
from modules.io_utils        import load_term_maps, load_nifti
from modules.stats_utils     import split_and_convert_t_to_z, compute_zmap_correlations, compute_difference_stats
...
def main():
    ...
    for filepath in glob(os.path.join(brain_dir, '*.nii')):
        ...
        # Correlation plots with significance and CI
        ...
        # Difference stats and plot
        diff_df = compute_difference_stats(
            z_pos, z_neg,
            term_maps, ref_img
        )
        diff_png = os.path.join(out_dir, f"diff_correlations_{run_id}.png")
        diff_csv = os.path.join(out_dir, f"diff_correlations_{run_id}.csv")
        plot_difference(
            diff_df,
            diff_png,
            subtitle,
