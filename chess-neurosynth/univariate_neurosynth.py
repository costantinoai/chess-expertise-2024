...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 14 03:17:36 2025
...
def remove_useless_data(data: np.ndarray, brain_mask_flat: np.ndarray = None):
    ...
    from a stack of flattened image data maps.
    ...
    # Combine the two masks: finite and non-flat
    ...
    # Final count of retained voxels
...
def load_and_mask_imgs(file_list):
    """
    Load NIfTI images, masking out voxels outside the brain mask.
...
def t_to_two_tailed_z(t_map: np.ndarray, dof: int) -> np.ndarray:
    """
    Convert a t-map to a two-tailed z-map, retaining the original sign.

    Parameters
    ----------
    t_map : np.ndarray
        Input t-statistic map.
    ...
    -------
    z_map : np.ndarray
        Signed two-tailed z-map, such that:
        - Positive t-values yield positive z
        - Negative t-values yield negative z
    """
    # Two-tailed p-values: P(|T| > |t|) = 2 * P(T > |t|)
    t_abs = np.abs(t_map)
    p_two_tailed = 2 * t.sf(t_abs, df=dof)

    # Compute unsigned z and reapply the sign of the original t-map
    z_abs = norm.isf(p_two_tailed / 2)
    z_map = np.sign(t_map) * z_abs

    # Set zero where t == 0 to avoid spurious tiny z-values
    z_map[t_abs == 0] = 0.0
    return z_map
...
def split_zmap_by_sign(z_map: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """
    Split a z-map into positive and negative effect maps.

    Parameters
    ----------
    z_map : np.ndarray
        One-tailed z-map (positive values only).

    Returns
    -------
    z_pos : np.ndarray
        z-map for positive t-values; others set to 0.

    z_neg : np.ndarray
        z-map for negative t-values (sign flipped to be positive); others set to 0.
    """
...
# --- Configuration ---
TERM_DIR     = 'data/terms'         # Path to Neurosynth meta-analytic maps
BRAIN_DIR    = 'data/smooth4'       # Path to input statistical brain maps
...
# Optional: mapping of abbreviations to human-readable names (if needed)
LEVELS_MAPS = {
    "exp": "Experts",
    "nov": "Novices",
    "gt": "Greater Than",
...
logger.info("=== Starting Neurosynth Correlation Analysis ===")
    ...
    for filepath in glob(os.path.join(BRAIN_DIR, '*.nii')):
        ...
        parts = run_id.replace("_gt_", ">").split('_')
        level1 = LEVELS_MAPS.get(parts[1], parts[1])
        level2 = LEVELS_MAPS.get(parts[2], parts[2])
        subtitle = f"{clean_levels[level2]}"
        ...
        plot_map(
            z_data, con_img,
            title=f'{subtitle} | Experts>Novices',
            outpath=os.path.join(RESULT_ROOT, f'{subtitle}_glassbrain_experts_gt_novices.png'),
        ...
        plot_surface_map(
            ...
            output_file=os.path.join(RESULT_ROOT, f'{subtitle}_surface_experts_gt_novices.png')
        )
        plot_surface_map_flat(
            con_img,
            title=f'{subtitle} | Experts>Novices',
            threshold=MIN_VOXEL_VALUE,
            output_file=os.path.join(RESULT_ROOT, f'{subtitle}_surface_flat_experts_gt_novices.png')
        )
        ...
        df_neg.to_csv(os.path.join(RESULT_ROOT, f'{subtitle}_term_corr_negative.csv'), index=False)
        df_diff.to_csv(os.path.join(RESULT_ROOT, f'{subtitle}_term_corr_difference.csv'), index=False)
        save_latex_correlation_tables(df_pos, df_neg, df_diff, out_dir=RESULT_ROOT, run_id=subtitle)

        # STEP 2i: Plot correlation strength and difference
        plot_correlations(
            df_pos, df_neg, df_diff,
            run_id=subtitle,
            out_fig=os.path.join(RESULT_ROOT, f'{subtitle}_term_correlations.png')
        )
        plot_difference(
            df_diff,
            run_id=subtitle,
            out_fig=os.path.join(RESULT_ROOT, f'{subtitle}_term_correlation_differences.png')
        )
